...
lemma BIT_bin_simps [simp]:
  ...
  unfolding numeral.simps numeral_BitM
     apply (simp_all add: Bit_B0)
  apply (metis Bit_B0 Bit_B0_2t Bit_B1_2t numeral_One numeral_plus_numeral)
  by (smt (verit, best) Bit_B1 minus_sub_one_diff_one numeral_plus_numeral)
...
proof(induction m arbitrary: x y n)
  case 0
  thus ?case
    by(simp add: bin_last_def shiftl_int_def) (metis (opaque_lifting, no_types) mod_diff_right_eq mod_self neq0_conv numeral_One power_eq_0_iff power_mod diff_zero zero_neq_numeral)
next
  case (Suc m)
